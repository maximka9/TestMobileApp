...
Мобильная 2D clicker/idle-игра про стримера SASAVOT. Версия **0.5.0**.
...
Усталость уменьшается на 2 пункта за полную минуту между эфирами. Кнопка «•••» открывает настройки: можно уменьшить анимацию
и вручную повторить сохранение после ошибки записи.

## Visual design

Изменения 0.5: отдельная кухня, дерево достижений, публичный каталог стримеров,
источники для TikTok, событие FAIL и бонус XP при хайпе 95+.
Данные, миграция и обновление каталога описаны в [v0.5-content.md](docs/v0.5-content.md).
...
Экран «Коллаб» подбирает 10 кандидатов из bundled-каталога на 500 вымышленных
профилей разных масштабов. Это production-scale набор для офлайн-игры;
имена и reference audience — игровые данные, а не публичная аналитика.
Выберите автора и формат: интерфейс покажет качественную оценку шанса и причины.
Ответ появляется после короткой локальной анимации. Успех даёт подписчиков,
отношения, репутацию и временное ускорение; оба исхода создают cooldown.
Данные и проверки фазы 4A: [docs/career_phase4a.md](docs/career_phase4a.md).
