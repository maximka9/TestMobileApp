...
static func create(state: PlayerState, tag: String, timestamp: int) -> void:
	state.source_sequence += 1
	state.content_sources.append({"id": state.source_sequence, "source_stream_id": str(state.total_streams + (1 if state.is_streaming else 0)), "tags": [tag], "created_at": timestamp, "consumed": false})
